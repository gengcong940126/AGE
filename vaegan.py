...
os.makedirs('./results_celeba/vaegan_128',exist_ok=True)
os.makedirs('./results_celeba/vaegan_128/tb',exist_ok=True)
writer=SummaryWriter(log_dir='./results_celeba/vaegan_128/tb')
...
# Load encoder
netE = load_vae_E(opt)
netd = load_vaegan_d(opt).to('cuda')
...
for epoch in range(opt.start_epoch, opt.nepoch):
    ...
    for i in range(len(dataloader['train'])):
        ...
        feature_AE,_=netd(AE)
        featrue_x,real=netd(x)
        err=match(feature_AE, featrue_x, opt.match_x)
        kl_loss = -0.5 * torch.sum(1 + logsigma - mu.pow(2) - logsigma.exp())/opt.batch_size
        loss = kl_loss+ err
        stats['AE_loss'] = err
