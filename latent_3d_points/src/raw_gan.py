...
class RawGAN(GAN):
  ...
  def __init__(self, name, learning_rate, n_output, noise_dim, discriminator, generator, beta=0.9, gen_kwargs={},
               disc_kwargs={}, graph=None):
  ...
  def _single_epoch_train(self, train_data, batch_size, noise_params={}, adaptive=None):
    ...
    if updated_d > 1:
      ...
      print
      'Discriminator was not updated in this epoch.'

    if adaptive is not None:
      print
      np.mean(_real_s), np.mean(_fake_s)
