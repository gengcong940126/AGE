...
def rand_rotation_matrix(deflection=1.0, seed=None):
  ...
  DOI: http://www.realtimerendering.com/resources/GraphicsGems/gemsiii
  /rand_rotation.c
       http://blog.lostinmyterminal.com/python/2015/05/12/random-rotation
       -matrix.html
  '''
  if seed is not None:
    np.random.seed(seed)
...
def unit_cube_grid_point_cloud(resolution, clip_sphere=False):
  ...
  If clip_sphere it True it drops the "corner" cells that lie outside the
  unit-sphere.
  '''
...
def plot_3d_point_cloud(x, y, z, show=True, show_axis=True, in_u_sphere=False,
                        marker='.', s=8, alpha=.8,
                        figsize=(5, 5), elev=10, azim=240, axis=None,
  ...
  if in_u_sphere:
    ...
    miv = 1.2 * np.min([np.min(x), np.min(y),
                        np.min(z)])  # Multiply with 0.7 to squeeze free-space.
